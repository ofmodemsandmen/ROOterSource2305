...
do_radio() {
	...
	two=$(echo "$chan" | grep "2.4 GHz, Channel 1)")
	if [ ! -z "$two" ]; then
		twoghz=$1
	else
		fiveghz=$1
...
if [ ! -z "$fiveghz" ]; then
	...
	while IFS= read -r line; do
		...
		ht40=$(echo "$line" | grep "HE40")
		if [ ! -z "$ht40" ]; then
			htmode="1"
		fi
		ht40=$(echo "$line" | grep "HE80")
		if [ ! -z "$ht40" ]; then
			htmode="2"
		fi
		ht40=$(echo "$line" | grep "HE160")
		if [ ! -z "$ht40" ]; then
			htmode="3"
		fi
		break
	...
	ht40=$(echo "$fivemode" | grep "HE40")
	if [ ! -z "$ht40" ]; then
		hmode="1"
	fi
	ht40=$(echo "$fivemode" | grep "HE80")
	if [ ! -z "$ht40" ]; then
		hmode="2"
	fi
	ht40=$(echo "$fivemode" | grep "HE160")
	if [ ! -z "$ht40" ]; then
		hmode="3"
	fi
	echo "$hmode" >> /tmp/wifisettings
